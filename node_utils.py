...
def tensor2pil_list(image):
    B,_,_,_=image.size()
    if  B==1:
        ref_image_list=[tensor_to_pil(image)]
    else:
        img_list = list(torch.chunk(image, chunks=B))
        ref_image_list = [tensor_to_pil(img) for img in img_list]
    return ref_image_list


def mask2pil_list(mask):
    B,_,_=mask.size()
    if  B==1:
        ref_image_list = [mask2pil(mask)]  
    else:
        img_list = list(torch.chunk(mask, chunks=B))
        ref_image_list = [mask2pil(img) for img in img_list]
    return ref_image_list

def tensor2pil_list_u(image,width,height):
    B,_,_,_=image.size()
    if  B==1:
...
def mask2pil_list_u(mask,width,height):
    B,_,_=mask.size()
    if  B==1:
        ref_image_list = [mask2pil_u(mask,width,height)]  
    else:
        img_list = list(torch.chunk(mask, chunks=B))
        ref_image_list = [mask2pil_u(img,width,height) for img in img_list]
    return ref_image_list

def mask2pil(mask):
    upscaled_mask =mask
    mask_np = upscaled_mask.squeeze(0).mul(255).clamp(0, 255).byte().numpy()
    return Image.fromarray(mask_np, mode='L')

def mask2pil_u(mask,width,height):
    upscaled_mask = tensor_upscale(mask.unsqueeze(-1), width, height)
    mask_np = upscaled_mask.squeeze(0).squeeze(-1).mul(255).clamp(0, 255).byte().numpy()
    return Image.fromarray(mask_np, mode='L')

def tensor2pil_RGBA_list(image,mask,width,height):
    B,_,_,_=image.size()
    # 处理mask维度
    if len(mask.shape) == 2:
