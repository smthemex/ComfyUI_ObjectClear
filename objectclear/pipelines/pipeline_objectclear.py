...
class ObjectClearPipeline(
    ...
    def __init__(
        ...
        self.image_prompt_encoder = None
    ...
    def from_pretrained_with_custom_modules(
        ...
        **kwargs,
        ...
        #     "F:/ComfyUI311/ComfyUI/custom_nodes/ComfyUI_ObjectClear/sdxl_repo",
        #     cache_dir=cache_dir,
        # )
        #image_prompt_encoder =None

        postfuse_module = PostfuseModule(embed_dim=2048, embed_dim_img=768) #2048
        ...
        #     cache_dir=cache_dir                     
        # )
        state_dict_postfuse = load_file(postfuse_path)
        postfuse_module.load_state_dict(state_dict_postfuse)
        del state_dict_postfuse
    ...
    def _encode_vae_image(self, image: torch.Tensor, generator: torch.Generator):
        ...
        if isinstance(generator, list):
            ...
            #print(generator.device,self.vae.device,image.device)
            image_latents = retrieve_latents(self.vae.encode(image), generator=generator) #TODO
    ...
    def __call__(
        ...
        **kwargs,
        ...
        #print(f"mask.shape: {mask.shape}",init_image.shape) #mask.shape: torch.Size([1, 1, 768, 512]) torch.Size([1, 3, 768, 512])
        if masked_image_latents is not None:
            ...
            if  self.image_prompt_encoder is not None:
                object_embeds = self.image_prompt_encoder(obj_only)
        #print(f"masked_image.shape: {masked_image.shape}",f"object_embeds.shape: {object_embeds.shape}") #masked_image.shape: torch.Size([1, 3, 768, 512]) object_embeds.shape: torch.Size([1, 1, 768])
